...
main = do
  ...
  window <- createWindow "Mandlebrot Set" defaultWindow
...
renderMandelbrot renderer limits maxIter = do
  ...
  let vec = V.unsafeFromForeignPtr0 fptr (fromIntegral $ winWidth * pitch)
      ...
      value x y = heatMapColor $ (fromIntegral m) / (fromIntegral maxIter)
        where V4 xMin yMin xMax yMax = limits
              fx = fromIntegral x
              fy = fromIntegral y
  ...
  sequence_ $ [writeVal (fromIntegral (y*pitch + x*4)) $ value x y |
    x <- [0..winWidth-1],
    y <- [0..winHeight-1]]
...
heatMapColor x = V3 r b g
    where r = round $ max 0 $ 255*(cos $ (3*pi/2) * (x**s - rp))
...
mandelbrot maxIter c = helper 0 0
  where helper n z = if n >= maxIter || magnitude z > 2
          then n
